...
! If this region is unlabelled, fall back to using the region's number
IF (DESCRIPTIONS(KNUM+1) == "UNLABELLED") THEN
    WRITE(DESCRIPTION, "(I)") KNUM
ELSE
    DESCRIPTION = DESCRIPTIONS(KNUM+1)
ENDIF

IF (KSWITCH == 0) THEN
  CALL ROCTXSTARTRANGE(DESCRIPTION)
